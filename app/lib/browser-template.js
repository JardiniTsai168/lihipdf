...
function xmlValue(value) {
  ...
  return normalized.split("\n").map((line) => escapeXml(line)).join("</w:t><w:br/><w:t>");
}
...
function applyTemplateSelectionState(documentXml, payload) {
  const replacements = [
    [[/[■□]第一型  [■□]第二型  [■□]第三型/g], payload.deviceTypeLine],
    [[/[■□]太陽光電  [■□]小水力  [■□]生質能  [■□]風力  [■□]地熱能  [■□]廢棄物  [■□]氫能  [■□]燃料電池  [■□]海洋能/g], payload.energyCategoryLine],
    [[/太陽光電-[■□]屋頂  [■□]地面  [■□]水面/g], `太陽光電-${payload.solarCategoryLine}`],
    [[/風力-[■□]陸域  [■□]離岸/g], `風力-${payload.windCategoryLine}`],
    [[/生質能-[■□]無  [■□]有厭氧消化設備  [■□]農林植物/g], `生質能-${payload.biomassCategoryLine}`],
    [[/廢棄物-[■□]一般  [■□]農業/g], `廢棄物-${payload.wasteCategoryLine}`],
    [[/[■□]併聯台電外線/g], checkboxLine(payload.parallelMethod, "台電外線", "併聯台電外線")],
    [[new RegExp(`[■□]併聯用戶內線，電號：${escapeRegex(payload.innerLineNumber)}`, "g")], checkboxLine(payload.parallelMethod, "用戶內線", `併聯用戶內線，電號：${payload.innerLineNumber}`)],
    [[/[■□]僅併聯不躉售/g], checkboxLine(payload.saleMode, "僅併聯不躉售", "僅併聯不躉售")],
    [[/[■□]全額躉售/g], checkboxLine(payload.saleMode, "全額躉售", "全額躉售")],
    [[/[■□]自發自用\(餘電躉售\)/g], checkboxLine(payload.saleMode, "自發自用(餘電躉售)", "自發自用(餘電躉售)")],
    [[/[■□]直供餘電躉售\(限第一型\)/g], checkboxLine(payload.saleMode, "直供餘電躉售(限第一型)", "直供餘電躉售(限第一型)")],
    [[/[■□]轉供餘電躉售/g], checkboxLine(payload.saleMode, "轉供餘電躉售", "轉供餘電躉售")],
    [[/[■□]轉供自用\(第二、三型\)/g], checkboxLine(payload.saleMode, "轉供自用(第二、三型)", "轉供自用(第二、三型)")]
  ];

  return replacements.reduce((xml, [[pattern], replacement]) => xml.replace(pattern, replacement), documentXml);
}

export async function renderOfficialDocxInBrowser(formData) {
